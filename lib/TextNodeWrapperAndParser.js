...
// TODO Use css highlight register
// So that all DOM highlights don't get cleared. 
// Only the highlights that are registered by this class.
// TODO Possibly make a class static? need to look into it. 
// TODO must remove range argument from constructor. .
class HighlightRangeText {
    ...
    constructor(backgroundColor, textColor) { 
        if (!this.#isHexColor(backgroundColor)) {
    ...
    currentText() {
        if(this.#injectedSpans === null) {
            throw new Error("No text has been wrapped, call wrapTextIn() first.");
        }
    ...
    highlightBetween(indexStart, indexEnd) { 
        ...
        else if(this.#injectedSpans === null) {
            throw new Error("No text has been wrapped, call wrapTextIn() first.");
        }else if(indexEnd > this.#injectedSpans.wrapperElms[this.#spanIndex].fullText.length) {
            throw new RangeError("Index range exceeds text length.");
        }
