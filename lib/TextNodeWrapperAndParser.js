...
class HighlighterWalker { // might not be needed
    ...
    setTextNodes(range) {
        this.#textNodesObj.nodes = HighlighterWalker.getTextNodes(range);
        this.#textNodesObj.index = 0;
    }

    static getTextNodes(range){ // todo handel single text node
        if (!range instanceof Range) {
            throw new TypeError(`Parameter range must be a Range object, ${typeof range} given.`);
        }else if(range.collapsed === true) {
            throw new Error('Parameter range is collapsed, range must be a non-collapsed range.');
        ...
        treeWalker = document.createTreeWalker(
            ...
            (node) => { 
                ...
                if(
                    ...
                    ) === true // Custom filter function, can fail if the function throws an error
                ){ 
                    filterFlag = NodeFilter.FILTER_ACCEPT;
                };
                return filterFlag;
