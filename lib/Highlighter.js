class Highlighter { // todo jsdocs
    ...
    setStyle(backgroundColor = 'inherit', color = 'inherit') {
        if((!Highlighter.isHexColor(color) && color !== 'inherit') || (!Highlighter.isHexColor(backgroundColor) && backgroundColor !== 'color')) {
    ...
    static parseTextNodes(range){ // bug handel single text node
        if (!range instanceof Range) {
            throw new TypeError(`Parameter range must be a Range object, ${typeof range} given.`);
