    ...
    *  1. Error messages
    *  2. JSDoc
    * 3. Safe uuid generation method
    */
class TextNodeWrapper {

    // private properties
    #allowedHtmlTags = ['span', 'div', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']; 
    /**
    ...
    set regexTextContent(regexTextContent) {
        if(!regexTextContent instanceof RegExp) throw new Error('SelectionElmInjector:property:regexTextContent: regex must be an instance of RegExp'); 
        this.#regexTextContent = regexTextContent;
    }

    set wrapperHtmlTag(wrapperHtmlTag) {
        if(!this.isValidateHtmlTag(wrapperHtmlTag)) throw new Error(`Invalid HTML tag string, accepted values are: ${this.#allowedHtmlTags}`);
    ...
    get _getAllowedHtmlTags() {
        return this.#allowedHtmlTags;
    }

    set _allowedHtmlTags(tags) {
        if(!Array.isArray(tags) && tags.length <= 0) {
            throw new Error('SelectionElmInjector:property:allowedHtmlTags: tags must be an array of strings');
        } else if (tags.some((tag) => {
            if(typeof tag !== 'string') return false;
            // check if the tag is a valid html tag
            const doc = domParser.parseFromString(`<${tag}>test</${tag}>`, 'text/html');
            const parseError = doc.documentElement.querySelector('parsererror');
            return parseError === null && parseError.nodeType !== Node.ELEMENT_NODE;
        })) {
            throw new Error('SelectionElmInjector:property:allowedHtmlTags: tags must be an array of strings');
        }
     ...
     */
    constructor(wrapperHtmlTag, regexTextContent, 
        textNodeFilter = (node) => {
            return true;
        }
    ) {
    ...
    injectTags(range) {
        ...
        if (!range instanceof Range) {
            throw new Error('SelectionElmInjector:method:injectTags:range must be an instance of Range');
        }else if(range.collapsed === true) {
            throw new Error('SelectionElmInjector:method:injectTags:range must not be collapsed');
        }
     ...
     */
    #wordRangesInStr(str) { // refactor name to #wordRangesInStr
        let array = [],
        match;
